...
import { getBookByBookID, getQuantityByBookID, removeBookByBookID, removeBookInventoryByBookID, updateBookInfoByBookID, updateBookInventoryByBookID } from "@prisma/client/sql";
...
export async function PUT(
  ...
  try {
    ...
    if (!updateQuantity) {
      // Check if the book was not found
      return NextResponse.json({ error: "Book not found in book inventory" }, { status: 404 });
    }

    const response = NextResponse.json(
      { message: "Book updated successfully", updatedBook, updateQuantity },
      { status: 200 }
    );
